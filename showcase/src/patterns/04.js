...
import cusmtomStyles from "./usage.css";
...
const Usage = () => {
  ...
  return (
    <div style={{ width: "100%" }}>
      <MediumClap onClap={handleClap} className={cusmtomStyles.clap}>
        <MediumClap.Icon className={cusmtomStyles.icon} />
        <MediumClap.Count className={cusmtomStyles.count} />
        <MediumClap.Total className={cusmtomStyles.total} />
      </MediumClap>
      {!!count && (
        <div className={styles.info}>{`You've received ${count} claps!`}</div>
      )}
    </div>
  );
};
