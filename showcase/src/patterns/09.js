...
const useClapState = (initialState = INITIAL_STATE) => {
  ...
  const defaultInitialState = useRef(initialState)
  ...
  const updateClapState = useCallback(() => {
    setClapState(({ count, countTotal }) => ({
      ...
      countTotal: count < MAXIMUM_USER_COUNT ? countTotal + 1 : countTotal,
    }));
  }, [count, countTotal]);
  ...
  const reset = useCallback(() => {
    if(prevCount !== count){
      setClapState(defaultInitialState.current)
      resetRef.current++
    }
  }, [prevCount, count, setClapState])

  return {clapState, updateClapState, getCounterProps, getToggleProps, reset, resetDep: resetRef.current};
};
...
const Usage = () => {
  const {clapState, updateClapState, getToggleProps, getCounterProps, reset, resetDep} = useClapState(userInitialState);
